...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<ResponseResult<String>> handleServiceException(ServiceException ex, HttpServletRequest request) {
        // 注入servletRequest，用于出错时打印请求URL与来源地址
        logError(ex, request);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.parseMediaType(MediaTypes.JSON_UTF_8));
        ResponseResult<String> result = new ResponseResult<>(false,
                ex.getMessage(),
                "",
    ...
    public final ResponseEntity<ResponseResult<String>> handleGeneralException(Exception ex, HttpServletRequest request) {
        logError(ex, request);
    ...
    public void logError(Exception ex) {
        ResponseResult<String> result = new ResponseResult<>(false,
                ...
                ex.getMessage(),
                HttpStatus.INTERNAL_SERVER_ERROR.value());
        logger.error(jsonMapper.toJson(result), ex);
    }
    ...
    public void logError(Exception ex, HttpServletRequest request) {
        ...
        ResponseResult<Map<String, String>> result = new ResponseResult<>(false,
                HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(),
                map,
                HttpStatus.INTERNAL_SERVER_ERROR.value());
